...
(defstruct ^{:doc "An emission is a mapping of an observation and a state"} emission :obs :state)
...
(defn initialize-backtrace [hmm]
  (zipmap (keys (:starts hmm)) (repeat "Start")))
...
(defn get-backpointer-for-one-state [emiss state prev-matrix-entry hmm]
  (argmax (for [s (keys (:starts hmm))]
	    [s
	     (* (get-smooth prev-matrix-entry s)
		(get-smooth (get-smooth (:transitions hmm) s) state))])))
...
(defn update-viterbi [emiss prob-matrix hmm]
  (let [aa (for [state (keys (:starts hmm))]
	     [state (get-max-prob-for-one-state
		     ...
		     hmm)])]
  (concat prob-matrix
	    (list (zipmap (map first aa) (map last aa))))))
...
(defn
  ...
  (do 
    (assert (hmm-equality-check hmm))
    (assert (seq? emiss-seq))
    (assert (map? (first emiss-seq))))
  (loop [prob-matrix (list (initialize-viterbi (vector (map :obs (first emiss-seq))) hmm))
	 path (list (initialize-backtrace hmm))
	 e (map #(vec (map :obs %)) ((ngram-window (:n hmm)) (rest emiss-seq)))]
    (if (= 0 (count e))
      [path prob-matrix]
  ...
  (let [hid (set (.split (slurp "../resources/hidden") "\n"))
    ...
    (let [trisup05 (supervised-train-HMM hid (/ 5 10) training 3)]
      (write-metrics-to-file
	   ...
	   "supervised"
	   "output/POS.txt"))
    (let [tri10 (supervised-train-HMM hid 1 training 3)]
      (write-metrics-to-file
